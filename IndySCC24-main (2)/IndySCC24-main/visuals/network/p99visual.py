...
plt.scatter(pre_data['Size'], pre_data['P99_Tail_Lat_us'], 
            color='blue', alpha=0.5, label='Default Settings')

# Calculate trendline for Pre data
z = np.polyfit(np.log10(pre_data['Size']), pre_data['P99_Tail_Lat_us'], 1)
p = np.poly1d(z)
...
plt.scatter(post_data['Size'], post_data['P99_Tail_Lat_us'], 
            color='green', alpha=0.5, label='Altered Settings')

# Calculate trendline for Post data
z = np.polyfit(np.log10(post_data['Size']), post_data['P99_Tail_Lat_us'], 1)
p = np.poly1d(z)
...
plt.scatter(mvapich_data['Size'], mvapich_data['P99_Tail_Lat_us'], 
            color='red', alpha=0.5, label='Altered w/ MVAPICH')

# Calculate trendline for MVAPICH data
z = np.polyfit(np.log10(mvapich_data['Size']), mvapich_data['P99_Tail_Lat_us'], 1)
p = np.poly1d(z)
...
plt.ylabel('P99 Tail Latency (μs)')
